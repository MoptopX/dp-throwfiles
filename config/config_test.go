...
func TestLoadEnv(t *testing.T) {
	...
	data := []byte("DEPI_DATABASE_DRIVER=\"sqlite3\"")
	ioutil.WriteFile("env_values", data, 0644)
	defer os.Remove("env_values")

	LoadEnv("env_values")

	got := os.Getenv("DEPI_DATABASE_DRIVER")
	if got != "sqlite3" {
...
func TestParse(t *testing.T) {
	...
	os.Setenv("DEPI_SECRET", secret)
	cfg = Parse()
	if cfg.Secret != secret {
		t.Errorf("Expected %#v, got %#v", secret, cfg.Secret)
	}

	os.Setenv("DEPI_DATABASE_DRIVER", "sqlite")
	cfg = Parse()
...
func TestDatabaseURL(t *testing.T) {
	data := []byte("DEPI_DATABASE_URL=\"postgres://dbuser:dbsecret@dbhost:1234/dbname\"")
	ioutil.WriteFile("env_values", data, 0644)
	defer os.Remove("env_values")
